...
func = Introspection.func
args = Introspection.args
import SymPyCore: SymFunction
...
##XXX    @test isa(N(p, 60), BigFloat)
    ...
    @test ↑(func(eqn)(map(↓, diff.(args(eqn)))...)) == Eq(2x + 2u(x) * diff(u(x),x), 3x^2 - diff(u(x),x))
...
#    if isdefined(SymPy, :mpmath)
    ...
    if VersionNumber(string(↓(sympy).__version__)) >= v"1.12.0"
...
@testset "Syms macro" begin
    ...
    @test isa(x, Sym)
    @test ask(𝑄.real(x) & 𝑄.positive(x)) == true
    @test string(x) == "x₀"

    @test isa(y, Sym)

    @test isa(z, Sym)
    @test ask(𝑄.complex(z)) == true

    @test isa(n, Sym)
    @test ask(𝑄.integer(n)) == true

    @syms f()::(real, positive), g(), h()::complex=>"h̄"
    @test isa(f, SymFunction)
    @test ask(𝑄.real(f(x)) & 𝑄.positive(f(x))) == true

    @test isa(g, SymFunction)

    @test isa(h, SymFunction)
    @test ask(𝑄.complex(h(x))) == true
    @test string(h) == "h̄"
...
@testset "Fix past issues" begin
    ...
    u = lambdify(Sym(1//2))
    @test u() ==  1/2
    @test u(1,2,3) == 1/2 # remove no arguments so an error
    @syms x
...
#    @test i2.args[1].args[1] == :square
    ...
    A = fill(Sym("a"), 2, 2)
    @test eltype(A*A) == eltype(A)
    @test eltype(A*ones(2,2)) == eltype(A)
    @test eltype(A*Diagonal([1,1])) == eltype(A)
    @test eltype(A * I(2)) == eltype(A)
...
@testset "generic programming, issue 223" begin
    ...
    @test α * M == M * α
    @test 2 * M == M * 2
    @test isa(M/α, Matrix{<:Sym})
    @test isa(α * inv(M), Matrix{<:Sym})

    ## issue #408 with inv
    @syms n::(integer,positive)
    A = sympy.MatrixSymbol("A", n, n)
    ## @test A.inv() == A.I ## XXX inv(A) == A.Ican't compare with equalis
    @test hash(A.inv()) == hash(A.I)
    # ceil broken
    @syms x
    ...
    ## test sympy.Indexed by comparing expressions
    u = sympy.Indexed(sympy.IndexedBase("x"), 1, -2)
    @test SymPyCore.convert_expr(u) == :(x[1, -2])
